...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Define a streaming query for Kafka
    val kafkaDataStreamWriter = finalPredictions
      .selectExpr("to_json(struct(*)) AS value") // Convert each row into a JSON string
      .writeStream
      .format("kafka")
      .option("kafka.bootstrap.servers", "kafka:9092") // Kafka broker
      .option("topic", "flight_delay_classification_response") // Topic name (same as MongoDB collection)
      .option("checkpointLocation", "/tmp/kafka") // Unique checkpoint location for Kafka
      .outputMode("append")


    // run the query
    val mongoQuery = dataStreamWriter.start()
    val kafkaQuery = kafkaDataStreamWriter.start()
